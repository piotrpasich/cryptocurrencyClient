...
class BtcTransporter extends HttpClient implements BlockchainTransporter
{
    public $cafile;
    /**
     * URL of the server
     ...
     * @var boolean
     */
    private $verifySslCertificate = false;
    ...
    {
        $this->url = sprintf("https://%s:%s@%s:%d",
            $config->getRpcUser(),
            $config->getRpcPass(),
            $config->getHost(),
            $config->getPort()
        );
        $this->cafile = $config->getCafile();
    }
